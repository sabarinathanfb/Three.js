...
function changeColorAtPosition(mesh, position, range, color) {
    const geometry = mesh.geometry;

    if (!geometry) {
        console.error('No geometry found for the mesh.');
        return;
    }
    ...
    for (let i = 0; i < positionAttribute.count; i++) {
        ...
        // Check if the vertex is within the specified range of the target position
        if (Math.abs(x - position.x) < range && Math.abs(z - position.z) < range) {
            // Set the vertex color
            colorAttribute.setXYZ(i, color.r, color.g, color.b);
            console.log(`Changing color of vertex ${i} to ${color.r}, ${color.g}, ${color.b}`); // Log change
        } else {
            // Optionally set a default color (like white) for other vertices
            colorAttribute.setXYZ(i, 1, 1, 1); // Default color (white)
        }
    }

    colorAttribute.needsUpdate = true; // Notify Three.js to update the colors
}


// Example usage:
const targetPosition = new THREE.Vector3(0, 0, 0);
const range = 100; // Change color within this range
const newColor = new THREE.Color(1, 0, 0); // Red color
...
function loadLayoutModel() {
    loader.load('./assets/Main_Layout.gltf', (gltf) => {
        ...
        changeColorAtPosition(layoutModel.children[0], targetPosition, range, newColor);
